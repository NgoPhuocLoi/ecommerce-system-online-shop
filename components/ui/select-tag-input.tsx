  ...
  ({ className, value, onChange, options, ...props }, ref) => {
      ...
      if (newOption) {
        ...
        const matchedOption = options.find(
          (option) =>
            ...
            pendingDataPoint.trim().toLowerCase(),
        );
    ...
    return (
      <div className={cn("relative", className)}>
        <div
          className={cn(
            "flex min-h-10 w-full flex-wrap gap-2 rounded-md border border-neutral-200 bg-white px-3 py-2 text-sm ring-offset-white disabled:cursor-not-allowed disabled:opacity-50 has-[:focus-visible]:outline-none has-[:focus-visible]:ring-1 has-[:focus-visible]:ring-neutral-950 has-[:focus-visible]:ring-offset-0 dark:border-neutral-800 dark:bg-neutral-950 dark:ring-offset-neutral-950 dark:has-[:focus-visible]:ring-neutral-300",
          )}
        >
          {value.map((val) => (
        ...
        {isDropdownOpen && pendingDataPoint && (
          <ul
            className="absolute left-0 mt-1 max-h-60 w-full overflow-auto rounded-md border border-neutral-200 bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:border-neutral-800 dark:bg-neutral-950 sm:text-sm"
            role="listbox"
          >
            {options.filter(
              (option) =>
